...
$disabled3= " disabled";
if($catalog != ""){
	$result = mysql_query("SELECT * FROM Catalog WHERE useFor='TYPE_PRO' AND Parent=$catalog ORDER BY Sort");
	$catalog_list2 = "";
	while($rs=mysql_fetch_array($result)){
...
if($catalog2 != ""){
	$result = mysql_query("SELECT * FROM Catalog WHERE useFor='TYPE_PRO' AND Parent=$catalog2 ORDER BY Sort");
	$catalog_list3 = "";
	while($rs=mysql_fetch_array($result)){
...
<table width="960" style="background:<?=$bg_main?>; width:960px; height:116px" cellpadding="0" cellspacing="0">
	...
	<tr>
    	<td style="height:45px; background:url('images/bg_content2.png'); background-repeat:repeat-y; background-position:center center; text-align:center; vertical-align:middle">
			<form name="sForm" action="member_product5.php" STYLE="margin: 0px; padding: 0px;">
