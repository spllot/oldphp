...
include 'seller_transfer_tab.php';
				...
				<tr>
					<td style="padding-bottom:20px">
						<table style="width:100%; background:#606060; border:solid 2px #606060" cellpadding="1" cellspacing="1">
							<tr>
								<td style="color:white; text-align:center; background:#909090">訂單編號</td>
								<td style="color:white; text-align:center; background:#909090; width:80px">數量</td>
								<td style="color:white; text-align:center; background:#909090; width:100px">訂單狀態</td>
								<td style="color:white; text-align:center; background:#909090; width:150px">月結匯款申請記錄</td>
							</tr>
							<tr>
								<td style="background:white; text-align:center"><a href="javascript:parent.Dialog('seller_logs_buyer.php?id={$data['ID']}&sort={$data['Sort']}');">{$data['ID']}</a></td>
								<td rowspan="2" style="background:white; text-align:center">{$data['Amount']}</td>
								<td style="background:white; text-align:center">{$status}</td>
		...
		<table border=0 width="700">
					...
					<table>
						<Tr>
							...
							<Td valign="top" style="color:gray">
								<table>
									<tr>
										<td style="color:gray">因各銀行匯款處理時間不同，月結匯款申請紀錄所示之日期，匯款最遲需三日入帳。</td>
									</tr>
								</table>
							</td>
						</tr>
			...
			<tr>
				<td align="left">
					<table>
						<Tr>
							<td>待處理{$tab_name}訂單<span style="color:gray; font-size:10pt">  ({$counts_t}</font>商品數處理，{$counts_d}</font>已完成)</span></td>
						</tr>
					</table>
				</td>
			</tr>
