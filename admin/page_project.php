...
if ($no > 0){
    $result=mysql_query("SELECT No, Code, Url, Content FROM Project WHERE No = '$no'");
    if(($num=mysql_num_rows($result))==1){
        list($no, $code, $url, $content) = mysql_fetch_row($result);
    }//if
	$c_dis = " DISABLED";
}//if
$init .= <<<EOD
...
$page->addContent("        <form name=\"iForm\" action=\"page_project_save.php\" method=\"post\">");
...
$page->addContent("                    <tr>");
$page->addContent("                        <td class=\"html_label_required\">專案代碼：</td>");
$page->addContent("                        <td align=\"left\"><input type='text' name='code' style='width:100px' value='$code'{$c_dis}></td>");
$page->addContent("                    </tr>");
$page->addContent("                    <tr>");
$page->addContent("                        <td class=\"html_label\">網址：</td>");
$page->addContent("                        <td align=\"left\"><input type='text' name='url' style='width:600px' value='$url'></td>");
$page->addContent("                    </tr>");
$page->addContent("                    <tr>");
