...
if ($itemno > 0 && $diff != 0){
	...
	$sql = "SELECT Sort FROM AD WHERE No = '$itemno'";
	$result = mysql_query($sql) or die (mysql_error());
	if (list($curr_sort) = mysql_fetch_row($result)){
		$max_sort = 0;
		$sql = "SELECT max(Sort) FROM AD WHERE useFor = '$usefor'";
		$result = mysql_query($sql) or die (mysql_error());
		list($max_sort) = mysql_fetch_row($result);
		$new_sort = $curr_sort + $diff;
		if ($new_sort >= 0 && $new_sort <= $max_sort){
			mysql_query("UPDATE AD SET Sort = '$curr_sort' WHERE Sort = '$new_sort' AND useFor = '$usefor'") or die (mysql_error());
			mysql_query("UPDATE AD SET Sort = '$new_sort' WHERE No = '$itemno'") or die (mysql_error());
		}
	}
