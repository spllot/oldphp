...
$page->addJSFile("../js/My97DatePicker/WdatePicker.js");
include("../include/db_open.php");
$date = date('Y-m-d');
if ($no > 0){
    $result=mysql_query("SELECT No, Subject, Content, Date FROM News WHERE No = '$no'");
    if(($num=mysql_num_rows($result))==1){
        list($no, $subject, $content, $date) = mysql_fetch_row($result);
    }//if
}//if
$init .= <<<EOD
...
$page->addContent("        <form name=\"iForm\" action=\"news_save.php\" method=\"post\">");
...
$page->addContent("                    <tr>");
$page->addContent("                        <td class=\"html_label_required\">日期：</td>");
$page->addContent("                        <td align=\"left\"><input type='text' name='date' value='{$date}' style='width:100px' onClick='WdatePicker();'></td>");
$page->addContent("                    </tr>");
$page->addContent("                    <tr>");
