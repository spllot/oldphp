...
$sql = "SELECT * FROM logScripts WHERE Script = 'sendmail.php' ORDER BY `Date` DESC LIMIT 1";
$result = mysql_query($sql);
$log = mysql_fetch_array($result);



$sql = "SELECT * FROM queueEMail WHERE 1=1";
if ($userid <> ""){
    $sql .= " AND (Recipient LIKE '%$userid%' OR Subject LIKE '%$userid%')";
}//if
$sql .= " ORDER BY dateRequested";
...
$page->addJSFile("../js/queue_admin.js");
...
$page->addContent("<TR class=\"grid_toolbar\"><TD>最近發送時間：{$log['Date']}，{$log['Result']}</td></tr>");
...
$page->addContent("<TH class=\"grid_heading\" style=\"width:200px\">收件人</TH>");
$page->addContent("<TH class=\"grid_heading\">標題</TH>");
$page->addContent("<TH class=\"grid_heading\" style=\"width:120px\">排程日期</TH>");
$page->addContent("<TH class=\"grid_heading\" style=\"width:80px\">發送次數</TH>");
$page->addContent("<TH class=\"grid_heading\" style=\"width:50px\">狀態</TH>");
$page->addContent("</TR>");
if ($num>0){
	...
	for ($i = 0; $i < $pagesize; $i++) {
		if($record = mysql_fetch_array($result)){
			$page->addContent("<TR>");
            $page->addContent("<TD class=\"grid_check\"><input type=\"checkbox\" name=\"memberno\" value=\"{$record['No']}\"></TD>");
			$page->addContent("<TD class=\"grid_no\">".(($pageno-1)*$pagesize + $i + 1)."</TD>");
            $page->addContent("<TD class=\"grid_center\">{$record['Recipient']}</TD>");
			$page->addContent("<TD class=\"grid_left\" style='padding-left:2px'><A HREF=\"javascript:Edit('{$record['No']}')\">{$record['Subject']}</A></TD>");
            $page->addContent("<TD class=\"grid_center\">{$record['dateRequested']}</TD>");
            $page->addContent("<TD class=\"grid_center\">{$record['Retry']}</TD>");
            $page->addContent("<TD class=\"grid_center\">" . (($record['Retry'] >= 3) ? "放棄" : "等待") . "</TD>");
			$page->addContent("</TR>");
		}//if
		else
			break;
