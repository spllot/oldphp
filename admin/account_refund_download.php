...
if($Y != "" && $M != "" && $D != ""){
	$sql = "SELECT *, Billing.No AS bNo,  DATEDIFF(CURDATE(), dateGenerate) AS Days, IFNULL((SELECT payBy FROM Payment WHERE Memo=Billing.orderID AND payBy <> 4), 0) as payBy1 FROM logBilling INNER JOIN Billing ON logBilling.No=Billing.logNo";
	$sql .= " WHERE Y='$Y' AND M='$M' AND D='$D' AND Apply=1 and logBilling.Refund=1";
	...
	if ($num>0){
		...
		$objPHPExcel->getActiveSheet()->setCellValue(chr(65 + $col) . "$row", "退款額");$col++;
		...
		while($rs = mysql_fetch_array($result)){
			...
			$objPHPExcel->getActiveSheet()->getCell(chr(65 + $col) . "$row")->setValueExplicit($rs['Paid'], PHPExcel_Cell_DataType::TYPE_STRING);$col++;
			$objPHPExcel->getActiveSheet()->getCell(chr(65 + $col) . "$row")->setValueExplicit($rs['Total'] + $rs['Fee'], PHPExcel_Cell_DataType::TYPE_NUMERIC);$col++;
			$objPHPExcel->getActiveSheet()->getCell(chr(65 + $col) . "$row")->setValueExplicit($payby[$rs['payBy1']], PHPExcel_Cell_DataType::TYPE_STRING);$col++;
			$row++;
			$total += $rs['Total'] + $rs['Fee'];
		}
		...
		$objPHPExcel->getActiveSheet()->setTitle('退款對帳檔');
		$objPHPExcel->setActiveSheetIndex(0);
		$objWriter = new PHPExcel_Writer_Excel5($objPHPExcel);
		$filename = "refund_" . date('Ymd') . ".xls";
