...
EOD;
			...
			if($_SESSION['Latitude'] > 0 && $_SESSION['Longitude']>0){
				...
				$dis = "距離：<a id='dis{$data['No']}' style='color:red'; href='javascript:void(0);' onMouseOver='showDis();setDisPos({$data['No']}); '> ? </a>";
			}
			$counts = 0;
...
$WEB_CONTENT .= <<<EOD
<div id="dis" style="position:absolute;display:none; width:120px; padding:0px; text-align:center; background:#ffffcc; border-radius: 8px; border:solid 5px gray; z-index:99" onMouseOver="showDis();" onMouseOut="hideDis();">
...
function setDisPos(x){
	...
	$("#dis").css({"top": p.top-5, "left": p.left-$("#dis").width()+30});
