...
document.write('.btn_E     { width: 22px; height: 22px; border: 1px solid buttonface; margin: 0; padding: 0; }\n');
document.write('.btn_EOver { width: 22px; height: 22px; border: 1px outset; }\n');
...
this.width =  "auto";
this.height = "auto";
this.bodyStyle = 'background-color: #FFFFFF; font-family: "標楷體"; font-size: 12pt;';
...
//  ['strikethrough','subscript','superscript','separator'],
    ...
    ['linebreak'],
//    ['HorizontalRule','Createlink','InsertImage','InsertFlash','InsertTable','htmlmode','separator'],
    ['HorizontalRule','Createlink','InsertImage', 'InsertTable','separator', 'InsertYoutube', 'InsertHTML','separator','htmlmode'],
...
this.btnList = {
    ...
    "underline":      ['Underline',            '底線',          'editor_action(this.id)',  'ed_format_underline.gif'],
    ...
    "justifyleft":    ['JustifyLeft',          '靠左對齊',       'editor_action(this.id)',  'ed_align_left.gif'],
    "justifycenter":  ['JustifyCenter',        '置中',     'editor_action(this.id)',  'ed_align_center.gif'],
    "justifyright":   ['JustifyRight',         '靠右對齊',      'editor_action(this.id)',  'ed_align_right.gif'],
    "orderedlist":    ['InsertOrderedList',    '編號',       'editor_action(this.id)',  'ed_list_num.gif'],
    "unorderedlist":  ['InsertUnorderedList',  '項目',      'editor_action(this.id)',  'ed_list_bullet.gif'],
    "outdent":        ['Outdent',              '減少縮排',    'editor_action(this.id)',  'ed_indent_less.gif'],
    "indent":         ['Indent',               '增加縮排',    'editor_action(this.id)',  'ed_indent_more.gif'],
    "forecolor":      ['ForeColor',            '文字顏色',         'editor_action(this.id)',  'ed_color_fg.gif'],
    "backcolor":      ['BackColor',            '背景顏色',   'editor_action(this.id)',  'ed_color_bg.gif'],
    "horizontalrule": ['InsertHorizontalRule', '橫線',    'editor_action(this.id)',  'ed_hr.gif'],
    "createlink":     ['CreateLink',           '超連結',    'editor_action(this.id)',  'ed_link.gif'],
    "insertimage":    ['InsertImage',          '插入圖片',       'editor_action(this.id)',  'ed_image.gif'],
    "insertflash":    ['InsertFlash',          'Insert Flash',       'editor_action(this.id)',  'ed_flash.gif'],
    "inserttable":    ['InsertTable',          '插入表格',       'editor_action(this.id)',  'insert_table.gif'],
    "insertyoutube":  ['InsertYoutube',        '插入YouTube影片',       'editor_action(this.id)',  'youtube_18x18.gif'],
    "inserthtml":     ['InsertHTML',           '插入HTML',       'editor_action(this.id)',  'html_18x18.gif'],
    "htmlmode":       ['HtmlMode',             '檢視 HTML 原始碼',   'editor_setmode(\''+objname+'\')', 'ed_html.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
      ...
      toolbar += '<button title="' +btnTitle+ '" id="_' +objname+ '_' +btnCmdID+ '" class="btn_E" onClick="' +btnOnClick+ '" onmouseover="if(this.className==\'btn_E\'){this.className=\'btn_EOver\'}" onmouseout="if(this.className==\'btn_EOver\'){this.className=\'btn_E\'}" unselectable="on"><img src="' +config.imgURL + btnImage+ '" border=0 unselectable="on"></button>';
...
function editor_action(button_id) {
  ...
  // FontSize
    ...
    editdoc.execCommand(cmdID,2,val);
  }
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
	  ...
	  /*
		var myTitle = "Insert Image";
		var myText = showModalDialog(_editor_url + "popups/insert_image2.html",
									 myTitle,      // str or obj specified here can be read from dialog as "window.dialogArguments"
									 "resizable: yes; help: no; status: no; scroll: no; ");
		if (myText) { editor_insertHTML(objname, myText); }
	   */
	...
	// insert flash
		...
		var myTitle = "插入 Flash";
		var myText = showModalDialog(_editor_url + "popups/insert_flash.html",
									 myTitle,      // str or obj specified here can be read from dialog as "window.dialogArguments"
									 "resizable: yes; help: no; status: no; scroll: no; ");
		if (myText) { editor_insertHTML(objname, myText); }
	}
    ...
    // insert youtube
    else if (cmdID.toLowerCase() == 'insertyoutube'){
	  var myTitle = "插入YouTube影片";
      var myText = showModalDialog(_editor_url + "popups/insert_youtube.html",
                                 myTitle,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
 	  if (myText) { 
    ...
    // insert youtube
    else if (cmdID.toLowerCase() == 'inserthtml'){
	  var myTitle = "插入HTML";
      var myText = showModalDialog(_editor_url + "popups/insert_html.html",
                                 myTitle,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
 	  if (myText) { 
			editor_insertHTML(objname, myText); 
...
function editor_updateToolbar(objname,action) {
  ...
  if (action == "enable" || action == "disable") {
    ...
    for (var idxN in tbItems) {
      ...
      if (action == "enable")  { tbObj.disabled = false; if (isBtn) { tbObj.className = 'btn_E' }}
  ...
  for (i=0; i<IDList.length; i++) {
    ...
    if (!cmdActive)  {                                  // option is OK
      if (btnObj.className != 'btn_E') { btnObj.className = 'btn_E'; }
