...
$sql = "SELECT *, (SELECT Name FROM Member WHERE userID = Message.Sender) AS sName FROM Message WHERE `To` = '" . $_SESSION['member']['userID'] . "' AND Type = '3' order by dateSent desc";
		...
		if($rs = mysql_fetch_array($result)){
			...
			$list .= <<<EOD
				<tr>
					...
					<td style="text-align:left"><a href="javascript:parent.Dialog('member_message_detail.php?no={$rs['No']}')" class="prompt">{$rs['Subject']}</a></td>
