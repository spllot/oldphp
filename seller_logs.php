...
$sql = "SELECT Orders.*, Items.No AS itemNo, Items.Amount AS Amount2, Items.Refund, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), '0') AS A1, IFNULL((SELECT payBy FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), 0) AS P1, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 0) AS A2, IFNULL((SELECT payBy FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 4) AS P2 FROM Orders INNER JOIN Items ON Orders.ID=Items.orderID WHERE Seller = '" . $_SESSION['member']['No'] . "' AND Deliver=$deliver AND Items.Amount > 0";


switch($status){
	case 1://待付款
		$sql1 = " AND Orders.Status = 0";
		$sql2 = " AND 1=2";
		break;
	case 2://待發貨
		$sql1 = " AND Orders.Status = 1 AND dateShipped='0000-00-00 00:00:00' AND logCertify.Refund = 0";
		$sql2 = " AND 1=2";
		break;
	case 3://待鑑賞
		$sql1 = " AND Orders.Status = 1 AND dateShipped<>'0000-00-00 00:00:00' AND datediff(Now(), dateShipped) <= 7 AND logCertify.dateVertify='0000-00-00 00:00:00' AND logCertify.Refund = 0 ";
		$sql2 = " AND 1=2";
		break;
	case 4://待消費
		$sql1 = " AND Orders.Status = 1 AND logCertify.dateUse = '0000-00-00 00:00:00' AND dateShipped<>'0000-00-00 00:00:00' AND datediff(Now(), dateShipped) > 7 AND logCertify.Refund = 0";
		$sql2 = " AND 1=2";
		break;
	case 5://已完成
		$sql1 = " AND Orders.Status = 1 AND logCertify.dateUse <> '0000-00-00 00:00:00' AND logCertify.Refund = 0";
		$sql2 = " AND 1=2";
		break;
	case 6://退訂中
		$sql1 = " AND 1=2";
		$sql2 = " AND Items.Refund = 1 AND Items.dateReturn = '0000-00-00 00:00:00'";
		break;
	case 7://已退款
		$sql1 = " AND 1=2";
		$sql2 = " AND Items.Refund = 1 AND Items.dateReturn <> '0000-00-00 00:00:00'";
		break;
	case 8://已取消
		$sql1 = " AND Orders.Status = 3";
		$sql2 = " AND 1=2";
		break;
}
$sql1 .= (($product != "") ? " AND Orders.Product='$product'" : "");
$sql2 .= (($product != "") ? " AND Orders.Product='$product'" : "");
...
$sql = "SELECT ifnull(COUNT(logCertify.Serial), 0) FROM Orders INNER JOIN logCertify ON logCertify.orderID=Orders.ID WHERE Orders.Deliver=$deliver AND Orders.Seller = '" . $_SESSION['member']['No'] . "'AND Orders.Status = 1 AND logCertify.dateUse <> '0000-00-00 00:00:00' AND logCertify.Refund = 0" . (($product != "") ? " AND Orders.Product='$product'" : "");
$result = mysql_query($sql) or die(mysql_error());
if($rs = mysql_fetch_array($result))
	$counts_d = $rs[0];
...
$sql = "(SELECT Orders.dateSubmited, Orders.Deliver, Orders.ID, Orders.Status, CONCAT(Orders.pName, ' (', substring(logCertify.Serial, 1, 7), '***)') AS pName, Orders.dateShipped, logCertify.Serial, logCertify.Refund, logCertify.dateVertify, logCertify.dateUse, '0000-00-00 00:00:00' as dateReturn, 1 AS Amount, '0000-00-00 00:00:00' AS dateRefund, 0 AS itemNo, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), '0') AS A1, IFNULL((SELECT payBy FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), 0) AS P1, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 0) AS A2, IFNULL((SELECT payBy FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 4) AS P2, 0 AS Transfer, logCertify.Expire FROM Orders INNER JOIN logCertify ON Orders.ID = logCertify.orderID WHERE logCertify.Refund=0 AND Orders.Seller = '" . $_SESSION['member']['No'] . "' AND Deliver=$deliver $sql1)
UNION ALL
(SELECT Orders.dateSubmited, Orders.Deliver, Orders.ID, Orders.Status, Orders.pName, Orders.dateShipped, '' as Serial, Items.Refund, '0000-00-00 00:00:00' as dateVertify, '0000-00-00 00:00:00' as dateUse, Items.dateReturn, Items.Amount, Items.dateRefund, Items.No AS itemNo, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), '0') AS A1, IFNULL((SELECT payBy FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), 0) AS P1, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 0) AS A2, IFNULL((SELECT payBy FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 4) AS P2, Items.Transfer , Items.Expire FROM Orders INNER JOIN Items ON Orders.ID = Items.orderID WHERE Items.Refund=1 AND Orders.Seller = '" . $_SESSION['member']['No'] . "' AND Deliver=$deliver $sql2)
order by dateSubmited DESC, Refund, Serial";

$sql = "(SELECT Orders.dateSubmited, Orders.Deliver, Orders.ID, Orders.Status, Orders.pName, Orders.dateShipped, logCertify.Serial, logCertify.Refund, logCertify.dateVertify, logCertify.dateUse, '0000-00-00 00:00:00' as dateReturn, 1 AS Amount, '0000-00-00 00:00:00' AS dateRefund, 0 AS itemNo, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), '0') AS A1, IFNULL((SELECT payBy FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), 0) AS P1, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 0) AS A2, IFNULL((SELECT payBy FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 4) AS P2, 0 AS Transfer, logCertify.Expire, '' as Certify FROM Orders INNER JOIN logCertify ON Orders.ID = logCertify.orderID WHERE logCertify.Refund=0 AND Orders.Seller = '" . $_SESSION['member']['No'] . "' AND Deliver=$deliver $sql1)
UNION ALL
(SELECT Orders.dateSubmited, Orders.Deliver, Orders.ID, Orders.Status, Orders.pName, Orders.dateShipped, '' as Serial, Items.Refund, '0000-00-00 00:00:00' as dateVertify, '0000-00-00 00:00:00' as dateUse, Items.dateReturn, Items.Amount, Items.dateRefund, Items.No AS itemNo, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), '0') AS A1, IFNULL((SELECT payBy FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), 0) AS P1, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 0) AS A2, IFNULL((SELECT payBy FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 4) AS P2, Items.Transfer , Items.Expire, Items.Certify FROM Orders INNER JOIN Items ON Orders.ID = Items.orderID WHERE Items.Refund=1 AND Orders.Seller = '" . $_SESSION['member']['No'] . "' AND Deliver=$deliver $sql2)
order by dateSubmited DESC, Refund, Serial";
...
if ($num>0){
	...
	for ($j = 0; $j < $pagesize; $j++) {
		if($data = mysql_fetch_array($result)){
			...
			if($data['Status'] == 2 || $data['Refund'] == 1){
				$c = explode(",", $data['Certify']);
				if($data['Certify'] != "" && sizeof($c) > 0){
					$p_name .= " (";
					for($i=0; $i<sizeof($c); $i++){
						$p_name .= substr($c[$i], 0, 7) . "***";
						if($i < sizeof($c)-1){
							$p_name .= ", ";
						}
					}
				...
				if($data['Status'] == 1){
					$p_name .= (($data['Serial'] != "") ? " (" . substr($data['Serial'], 0, 7) . "***)" : "");
				}
				else{
					$p_name .= " (**********)";
				}
					...
					<td style="padding-bottom:20px">
						<table style="width:100%; background:#606060; border:solid 2px #606060" cellpadding="1" cellspacing="1">
							<tr>
								...
								<td style="color:white; text-align:center; background:#909090; width:60px">數量</td>
							...
							<tr>
								<td rowspan="2" style="background:white; text-align:center"><a href="javascript:parent.Dialog('seller_logs_buyer.php?id={$data['ID']}');">{$data['ID']}</a></td>
								<td rowspan="2" style="background:white; text-align:left; padding:2px">{$p_name}</td>
								<td rowspan="2" style="background:white; text-align:center">{$data['Amount']}</td>
