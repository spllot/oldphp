...
if($data['Mode'] == 1 && $data['Deliver'] == 0) $tab = 1;
if($data['Mode'] == 1 && $data['Deliver'] == 1) $tab = 2;
if($data['Mode'] == 2 && $data['Deliver'] == 0) $tab = 4;
if($data['Mode'] == 2 && $data['Deliver'] == 1) $tab = 5;
...
if ($num>0){
		if($data){
			...
			$city = (($data['M1'] > 0) ? $data['Area1'] : $data['City']);
...
EOD;
			if($data['Cashflow'] == 1){
				$onclick = ((!empty($_SESSION['member'])) ? "parent.Dialog('buynow.php?id={$data['No']}');":"window.location.href='member_login.php?url=" . urlencode("product4_detail.php?no={$data['No']}") . "';");
