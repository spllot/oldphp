...
$deliver=1;
$sql = "SELECT DISTINCT Product.No, Product.Name FROM Product INNER JOIN Orders ON Orders.Product = Product.No WHERE Product.Deliver=$deliver  AND Orders.Seller='" . $_SESSION['member']['No'] . "' AND Product.Receipt=1 ORDER BY Name";
...
if ($num>0){
	...
	for ($i = 0; $i < $pagesize; $i++) {
		if($data = mysql_fetch_array($result)){
			...
			if($status == "已完成"){
				if($data['Receipt'] == 1){
					$date1 = '<input type="text" id="s' . $data['No'] . '" style="width:80px" name="s' . $data['No'] . '" value="' . (($data['dateReceipt'] != "0000-00-00 00:00:00") ? substr($data['dateReceipt'], 0, 10) : "") . '">';
					$js1 = <<<EOD
						<script language="javascript">
							$('#s{$data['No']}').datepicker({
								dateFormat: "yy-mm-dd",
								timeFormat: "HH:mm",
								closeText: '確定',						
					...
					<td style="padding-bottom:20px">
						<table style="width:100%; background:#606060; border:solid 2px #606060" cellpadding="1" cellspacing="1">
							<tr>
								...
								<td style="color:white; text-align:center; background:#909090; width:100px;">發票開出日</td>
							</tr>
							<tr>
								...
								<td rowspan="2" style="background:white; text-align:center;">{$date1}</td>
							</tr>
						</table>{$js1}{$js2}
					</td>
				</tr>
		...
		<table border=0 width="700">
							...
							<Td valign="top" style="color:gray">
								<table>
									<tr>
										...
										<td style="color:gray">到店商品之發票或收據, 在消費者到店消費商品時, 將由商家直接開立之。</td>
									</tr>
									<tr>
										<td style="color:gray" nowrap valign="top">(2)</td>
										<td style="color:gray">宅配商品之收據處理, 可隨商品寄送消費者; 宅配商品之發票處理, 應於訂單”已完成”狀態後, 一周內寄達消費者手中。</td>
									</tr>
								</table>
							</td>
						</tr>
			...
			<tr>
				<td align="left">
					<table>
						<Tr>
							<td>待處理宅配發票<span style="color:gray; font-size:10pt">  ({$num}</font>訂單編號)</span></td>
						</tr>
					</table>
				</td>
			</tr>
...
function Receipt(xNo){
	var s = $("#s" + xNo).val();
	if(s != ""){
		$.post(
			'seller_receipt1_save.php',
			{
				s: s,
				no: xNo
			},
