...
$counts_r = 0;//已退款
$counts_d = 0;//已完成
$sql = "SELECT ifnull(SUM(Amount), 0) FROM Orders WHERE Orders.Deliver=$deliver AND Orders.Seller = '" . $_SESSION['member']['No'] . "' AND Orders.Status NOT IN(0, 3)" . (($product != "") ? " AND Orders.Product='$product'" : "");
$result = mysql_query($sql) or die(mysql_error());
if($rs = mysql_fetch_array($result))
	$counts_t = $rs[0];

$sql = "SELECT ifnull(SUM(Amount), 0) FROM Orders WHERE Orders.Deliver=$deliver AND Orders.Seller = '" . $_SESSION['member']['No'] . "' AND Status=3 AND Orders.Status NOT IN(0, 3)" . (($product != "") ? " AND Orders.Product='$product'" : "");
$result = mysql_query($sql) or die(mysql_error());
if($rs = mysql_fetch_array($result))
	$counts_c = $rs[0];

$sql = "SELECT ifnull(SUM(Items.Amount), 0) FROM Orders INNER JOIN Items ON Items.orderID=Orders.ID WHERE Orders.Deliver=$deliver AND Orders.Seller = '" . $_SESSION['member']['No'] . "' AND Items.Refund = 1 AND Items.dateReturn <> '0000-00-00 00:00:00' AND Orders.Status NOT IN(0, 3)" . (($product != "") ? " AND Orders.Product='$product'" : "");
$result = mysql_query($sql) or die(mysql_error());
if($rs = mysql_fetch_array($result))
	$counts_r = $rs[0];

$sql = "SELECT ifnull(SUM(Items.Amount), 0) FROM Orders INNER JOIN Items ON Items.orderID=Orders.ID WHERE Orders.Deliver=$deliver AND Orders.Seller = '" . $_SESSION['member']['No'] . "' AND Orders.Status = 1 AND dateShipped<>'0000-00-00 00:00:00' AND datediff(Now(), dateShipped) > 10 AND Items.Refund = 0 AND Orders.Status NOT IN(0, 3)" . (($product != "") ? " AND Orders.Product='$product'" : "");
$result = mysql_query($sql) or die(mysql_error());
if($rs = mysql_fetch_array($result))
	$counts_d = $rs[0];
...
$sql = "SELECT Orders.*, Items.No AS itemNo, Items.Sort, Items.dateBack, Items.dateReturn, Items.Amount AS Amount2, Items.Refund, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), '0') AS A1, IFNULL((SELECT payBy FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), 0) AS P1, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 0) AS A2, IFNULL((SELECT payBy FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 4) AS P2, Items.Transfer, Items.Expire FROM Orders INNER JOIN Items ON Orders.ID=Items.orderID WHERE Seller = '" . $_SESSION['member']['No'] . "' AND Deliver=$deliver AND Items.Amount > 0 AND Orders.Status NOT IN(0, 3)";
...
if ($num>0){
	...
	for ($i = 0; $i < $pagesize; $i++) {
		if($data = mysql_fetch_array($result)){
			...
			$date = '<input type="text" style="width:80px" onClick="WdatePicker();">';


			$date1 = '<input type="text" id="s' . $data['itemNo'] . '_' . $data['Sort'] . '" style="width:80px" name="s' . $data['itemNo'] . '_' . $data['Sort'] . '" value="' . (($data['dateShipped'] != "0000-00-00 00:00:00") ? substr($data['dateShipped'], 0, 10) : "") . '">';
			
			$date2 = '<input type="text" id="r' . $data['itemNo'] . '_' . $data['Sort'] . '" style="width:80px" name="r' . $data['itemNo'] . '_' . $data['Sort'] . '" value="' . (($data['dateBack'] != "0000-00-00 00:00:00") ? substr($data['dateBack'], 0, 10) : "") . '">';
			
			$js1 = <<<EOD
				<script language="javascript">
					$('#s{$data['itemNo']}_{$data['Sort']}').datepicker({
						dateFormat: "yy-mm-dd",
						timeFormat: "HH:mm",
						closeText: '確定',						
						onClose: function(dateText, inst) {
							Ship({$data['itemNo']}, {$data['Sort']});
						}
					});
				</script>
EOD;
			$js2 = <<<EOD
				<script language="javascript">
					$('#r{$data['itemNo']}_{$data['Sort']}').datepicker({
						dateFormat: "yy-mm-dd",
						timeFormat: "HH:mm",
						closeText: '確定',						
						onClose: function(dateText, inst) {
							Return({$data['itemNo']}, {$data['Sort']});
						}
					});
				</script>
EOD;
			$s1="";
			$s2="";
			if($data['dateShipped'] != "0000-00-00 00:00:00" || $data['Refund'] == 1){
				//隱藏出貨日
				$s1 = "none";
				$js1 = "";
				if($data['Refund'] == 0){
...
//				$date2 = $data['Refund'];
				...
				$js2 = "";
				if($data['dateShipped'] != "0000-00-00 00:00:00"){
					$date1 = substr($data['dateShipped'], 0, 10);
				}
			}
			$WEB_CONTENT .= <<<EOD
				<tr>
					<td style="padding-bottom:20px">
						<table style="width:100%; background:#606060; border:solid 2px #606060" cellpadding="1" cellspacing="1">
							<tr>
								...
								<td style="color:white; text-align:center; background:#909090; width:100px; display:$s1">出貨日</td>
								<td style="color:white; text-align:center; background:#909090; width:100px; display:$s2">回收貨日</td>
							</tr>
							<tr>
								...
								<td rowspan="2" style="background:white; text-align:center; display:$s1">{$date1}</td>
								<td rowspan="2" style="background:white; text-align:center; display:$s2">{$date2}</td>
							</tr>
						</table>{$js1}{$js2}
					</td>
				</tr>
		...
		<table border=0 width="700">
			...
			<tr>
				<td align="left">
					<table>
						<Tr>
							<td>待處理{$tab_name}訂單<span style="color:gray; font-size:10pt"> ({$counts_t}</font>商品數處理，{$counts_r}</font>已退款，{$counts_d}</font>已完成)</span></td>
						</tr>
					</table>
				</td>
			</tr>
...
function Ship(xNo, xItem){
	var s = $("#s" + xNo + "_" + xItem).val();
	if(s != ""){
		$.post(
			'seller_orders2_ship.php',
			{
				s: s,
				no: xNo,
				sort: xItem
...
function Return(xNo, xItem){
	var r = $("#r" + xNo + "_" + xItem).val();
	if(r != ""){
		$.post(
			'seller_orders2_return.php',
			{
				s: r,
				no: xNo,
				sort: xItem
