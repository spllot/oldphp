...
$sql = "SELECT logCertify.Refund, Orders.*, Items.Amount AS Amount2, Items.Refund, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), '0') AS A1, IFNULL((SELECT payBy FROM Payment WHERE payBy<>4 AND Memo=Orders.ID), 0) AS P1, IFNULL((SELECT Amount + Fee FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 0) AS A2, IFNULL((SELECT payBy FROM Payment WHERE payBy=4 AND Memo=Orders.ID), 4) AS P2 FROM Orders INNER JOIN Items ON Orders.ID=Items.orderID WHERE Member = '" . $_SESSION['member']['No'] . "' AND Deliver=$deliver AND Items.Amount > 0";
				...
				<tr>
					<td style="padding-bottom:20px">
						<table style="width:100%; background:#606060; border:solid 2px #606060" cellpadding="1" cellspacing="1">
							...
							<tr>
								<td rowspan="2" style="background:white; text-align:center"><a href="javascript:parent.Dialog('orders_detail.php?id={$data['ID']}');">{$data['ID']}</a></td>
